...
**FuncMapProvider** — supplies custom template functions; discovered automatically on ViewModel fields at compile time; `FuncMap() FuncMap`

**LayoutProvider** — declares a ViewModel's layout wrapper; the returned Handler must define `{{outlet}}`; cannot also implement RouterProvider; `Layout() Handler`

**Loader** — populates ViewModel fields from the incoming request; traversed depth-first post-order (children before parents); `Load(req) error`
...
**SelfValidator** — runs validation after form/query/path decoding; implemented by input structs, not ViewModels; `Validate(ctx) error`
...
**TemplateProvider** — supplies the HTML template string for a ViewModel or a nested sub-template; `Template() string`

## Core types

**Handler** — the compiled `http.Handler` for a ViewModel type; wraps template, router, layout, and stylesheet logic; created by `NewHandler[T]` or `MustNewHandler[T]`

**Middleware** — `func(http.Handler) http.Handler`; wraps a handler to intercept requests before and after dispatch

**PageLayoutViewModel** — the built-in root layout; renders the full HTML document shell (`<html>`, `<head>`, `<body>`); registered automatically by `NewRouter`; collects stylesheets, script tags, and inline styles from context
...
**ViewModel** — any Go struct; acts as both the HTTP response data and the handler configuration through interface implementation; declared as `type T = any` in the package
...
**`NewHandler[T]`** — constructs a Handler for a ViewModel type; compiles the template, configures the internal router via RouterProvider, applies LayoutProvider, detects StyleSheetProvider

**`NewRouter`** — creates the root Router; wraps all registered routes with PageLayoutViewModel by default; pass `DisableRootLayout()` to opt out
...
**`ProvideInlineStyles`** — adds `<style>` tags to the accumulator in request context; collected by PageLayoutViewModel at render time
...
**`template:"name"`** — on a ViewModel field that implements TemplateProvider, overrides the sub-template name used in `{{template "name"}}` calls; defaults to the field name

## Composition patterns

**Nested loaders** — a ViewModel field that itself implements Loader; torque discovers and hydrates these depth-first before calling Load on the parent; enables composable data-fetching sub-components
