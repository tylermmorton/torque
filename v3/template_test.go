...
func (*tmplWithFuncMap) FuncMap() torque.FuncMap {
...
func TestCompileTemplate(t *testing.T) {
	t.Run("flat_struct_compiles", func(t *testing.T) {
		_, err := torque.CompileTemplate(&tmplL1{})
		require.NoError(t, err)
	})

	t.Run("nested_3_deep_compiles", func(t *testing.T) {
		_, err := torque.CompileTemplate(&tmplL3Root{})
		require.NoError(t, err)
	})

	t.Run("nested_5_deep_compiles", func(t *testing.T) {
		_, err := torque.CompileTemplate(&tmplL5Root{})
		require.NoError(t, err)
	})

	t.Run("recurses_through_non_provider_container", func(t *testing.T) {
		tmpl, err := torque.CompileTemplate(&tmplPtRoot{})
		require.NoError(t, err)
		var buf bytes.Buffer
		require.NoError(t, tmpl.Render(&buf, nil, torque.TemplateRenderOptionTargets("pt-leaf")))
		require.Equal(t, `<leaf/>`, buf.String())
	})

	t.Run("struct_tag_names_the_subtemplate", func(t *testing.T) {
		tmpl, err := torque.CompileTemplate(&tmplTaggedRoot{})
		require.NoError(t, err)
		var buf bytes.Buffer
		require.NoError(t, tmpl.Render(&buf, nil, torque.TemplateRenderOptionTargets("custom-name")))
		require.Equal(t, `<leaf/>`, buf.String())
	})

	t.Run("func_map_provider_functions_are_registered", func(t *testing.T) {
		tmpl, err := torque.CompileTemplate(&tmplWithFuncMap{})
		require.NoError(t, err)
		var buf bytes.Buffer
	...
	t.Run("invalid_syntax_returns_error", func(t *testing.T) {
		_, err := torque.CompileTemplate(&tmplInvalid{})
		require.Error(t, err)
	})

	t.Run("custom_delimiters_parse_alternate_syntax", func(t *testing.T) {
		tmpl, err := torque.CompileTemplate(&tmplCustomDelims{}, torque.TemplateCompilerOptionDelims("[[", "]]"))
		require.NoError(t, err)
		var buf bytes.Buffer
...
func TestRender(t *testing.T) {
	...
	t.Run("named_target_renders_subtemplate_only", func(t *testing.T) {
		...
		require.NoError(t, err)
		var buf bytes.Buffer
		require.NoError(t, tmpl.Render(&buf, nil, torque.TemplateRenderOptionTargets("l3-leaf")))
		require.Equal(t, `<leaf/>`, buf.String())
	})
	...
	t.Run("multiple_targets_concatenated_in_order", func(t *testing.T) {
		...
		var buf bytes.Buffer
		require.NoError(t, tmpl.Render(&buf, nil,
			torque.TemplateRenderOptionTargets("l3-leaf", "l3-mid"),
		))
	...
	t.Run("funcmap_option_overrides_at_render_time", func(t *testing.T) {
		...
		var buf bytes.Buffer
		require.NoError(t, tmpl.Render(&buf, nil,
			torque.TemplateRenderOptionFuncMap(torque.FuncMap{
	...
	t.Run("unknown_target_returns_error", func(t *testing.T) {
		...
		require.NoError(t, err)
		var buf bytes.Buffer
		err = tmpl.Render(&buf, nil, torque.TemplateRenderOptionTargets("does-not-exist"))
		require.Error(t, err)
	})
